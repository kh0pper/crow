...
- [ ] **Step 2: Run** `node scripts/run-suite.mjs tests/peer-profile.test.js tests/handshake-display-name.test.js` → the new tests FAIL — all of them, the R1-C1 gate test included: its `peer_*` assertions would pass vacuously before Task 2 (nothing writes them yet), but its R2-S1 assertions on the placeholder `display_name` write fail until the gate exists (scoped check #2 observed 7 of 19 failing).
...
### Scoped check — 2026-09-08, sonnet, ISOLATED mirror (rsync without `.git`, own `git init`; the first scoped check ran in a `cp -a` mirror that shared the worktree's gitdir and verified round 2's stale commits — void, see the memory note `crow-review-mirror-shares-worktree-gitdir`), plan implemented verbatim from its text
**Verdict: PASS.** avatar 3, contact-display 3, peer-columns 1 → 7/7; Task 2 set 65/65; Task 3 set 44/44; Task 4 set 75/75 + import smoke `ok`; Task 5 set 41/41; Task 6 set 83/83 — full suite **4240/4240/0** (the plan's arithmetic exactly); check-ports OK; `build-registry --check` in sync. Invariants: `static/ramble.js` backticks 0, sinks 2, no emoji; `contactsClientJs()` no backtick / no `${`; exactly one `UPDATE contacts SET peer_` site; 0.8.0 in manifest, package and registry; the touched-file list matches the File structure section, nothing extra. Round-2 spot checks confirmed: `isEstablishedContact` in the receiver, the recipient list and `handleHandshakeComplete` (hoisted over the `display_name` write); `__profile_broadcast_pending` written by `broadcastProfile`, read by `refreshBirdAvatar` and `save_profile`; `save_profile` returns `broadcast` un-awaited; `avatarFieldValue` in `edit_contact` and `_applyContact`. No code deviation was needed; one stale expectation line (Task 2 Step 2) corrected afterwards. Awaiting Kevin's approval to execute.

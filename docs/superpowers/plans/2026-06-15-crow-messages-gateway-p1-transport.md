...
Executed subagent-driven (fresh implementer per task + reviews). All 8 tasks SHIPPED on `feat/crow-messages-gateway`; **20/20** Plan-1 tests + **38/38** regression (fix-it, exposure, project-spaces, auth-network invariant). Deltas from the plan, all from review findings (each fixed + tested):
- **Commit-review HIGH** → bot invites now **ed25519-signed** (was a dead HMAC); the whole payload (secp key/token/relays) is tamper-evident via the crowId-bound pubkey. (`ffd4b6a`)
- **Transport-core code-review I1** → added **persistent event dedup** (`bot_message_seen` + `markEventSeen`/`pruneSeen`, wired in `start()` before queueing) so a post-restart 24h relay replay can't re-run chat pi turns. (`dd877d3`)
- **T7 circular import** → `index.mjs → crow-messages.mjs → bridge.mjs → index.mjs` (real; `bridge.mjs` imports `index.mjs`) surfaced a TDZ once nostr-client's top-level `await` made the graph async-phased. Fixed by lazy-importing `bridge.mjs` inside `start()` (matches the other dynamic imports there); `handleCrowMessageEvent` is DI'd so it's unaffected. (`cd10abf`)
- **Final-review I1** → the bot-builder gateways save persisted ANY `gw_type` via a catch-all `else`, and "coming soon" options were selectable → crow-messages could be switched on and hosted before Plan 2's UI. Gated server-side (explicit gmail branch + ignore unsupported types) + render unavailable options `disabled`. Contained (default-deny) but a not-ready feature shouldn't be activatable. (`7d428c2`)

Two independent code reviews (transport-core + final integration) returned **no Critical**; the security boundary (authorize on the signed `event.pubkey` only, default-deny, no token double-burn, replies from the bot key, no `messages`-table write, no secret logging) was confirmed. Next: `/security-review` → finish/merge → Plan 2 (sharing UX).

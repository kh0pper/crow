...
- [ ] **Step 3: Implement the deletions** in the order: bot-board-api (execute/plan-dispatch/unstrand/force-unlock call + stage imports), board-stages.js + wrapper script, bridge (all FOUR sites), job_runner unstrand. Exit greps, BOTH must come back empty outside tests: `grep -rn "board-stages\|effectiveStage\|statusToStage\|stageToStatus\|isStage\b" --include="*.js" --include="*.mjs" servers scripts` AND — because two of the four bridge sites are raw SQL, invisible to the identifier grep (review critical #1) — `grep -rn "stage=\|stage='\|SET stage\|, stage" --include="*.js" --include="*.mjs" servers scripts | grep -vi "stagger"`.
- [ ] **Step 4: Update/retire the listed test files.** `bot-jobs-stranding.test.js`: delete — its concern (cards stuck in executing) is structurally impossible now and case 1/4 above are the proof. Run: `npm test -- tests/board-dispatch-job-rail.test.js tests/board-plan-dispatch.test.js tests/board-card-api.test.js tests/board-job-lock.test.js tests/board-stages-migration.test.js tests/board-plan-ref.test.js tests/cards-db-file-uri.test.js` → PASS. Mutation-test case 1 (re-add a status write to execute → fails).

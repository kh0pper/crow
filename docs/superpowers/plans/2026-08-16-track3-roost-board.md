...
- [ ] **Step 3: Implement.** In `board-lock.js`:
  ...
  - `lockedCardIds` (line 126): inner `MAX(id)` subquery gains `AND kind != 'perch-live'` — both the inner and the outer reads must exclude, or an excluded newest row shadows an older active one.
...
**Interfaces:**
- Produces: `cardBriefBlock({ cardId, tasksDbPath, planForCard, cardStatus, boardVocab })` → string — exactly the text bridge.mjs builds between `"Work the following card"` and `"One card only."` today, byte-identical. Bridge and the engine dispatch (Task 7) both call it; `projectHeader`, `gatewayHint`, and the `User said:` line stay caller-side (spec §5.1: parameterized inputs).
...
```
  ...
  NOTE the block ENDS before `" Then reply with a short summary for the gateway thread. One card only."` — that tail is channel-specific (per-rail reply semantics) and stays in bridge.mjs. Verify the exact current text against `bridge.mjs:688-695` when writing the fixture; the fixture is the law thereafter.
- [ ] **Step 2: Run — FAIL** (module missing).
- [ ] **Step 3: Implement** `card-brief.mjs`: a pure function assembling that string from the injected helpers; no imports from bridge (avoids the cycle). Then edit `bridge.mjs`'s card branch to `promptText = projectHeader + "\n\n" + cardBriefBlock({cardId, tasksDbPath, planForCard, cardStatus, boardVocab}) + " Then reply with a short summary for the gateway thread. One card only."` — wait: the historical text runs `…not a status write. Then reply…` with a single space; preserve byte-identity by making the brief end at `…not a status write.` and the bridge append `" Then reply with a short summary for the gateway thread. One card only."` and the `\n\nUser said: "…"\n\n` line **before** the brief exactly as today. Diff the composed prompt against the pre-change string in the test.
...
1. **Track model:** in `onChildEvent` (:647-671), add a `model_select` case: set `s.currentModel = m.provider + "/" + m.modelId` (match the event's real field names against `rpc-types.d.ts` / a live fixture first), update `s.resolved = {...s.resolved, provider: m.provider, model: m.modelId, key: s.currentModel}` so `onTurnEnd`'s `meterTurn({resolved: s.resolved})` (:727-731) prices the serving model, emit a state event and a `{type:"log", text:"now on <key>"}` system note.
2. **Apply model at wake:** in `startChild` (:551), after `prepareSpawn`, if `s.currentModel` is set and differs from `prep.resolved.key`, resolve it via the models list previously fetched (store `s.currentModelParts = {provider, modelId}` when tracking) and override `prep.piRpcOpts.resolved = {...prep.resolved, provider, model: modelId, key: s.currentModel}` before constructing PiRpc — the child spawns WITH the tracked model (spec §4.1.2: pi's session restore is CLI-overridden, so this is the source of truth).
...
- [ ] **Step 1: Failing tests** (fake PiRpc records sent frames; fake seams' `warmModel` records calls):
  - model_select event updates snapshot().model and the meterTurn call's `resolved.key` for the NEXT turn end.
  - control model switch calls warmModel BEFORE commandSince (assert call order).
  - hibernate → message (wake): fake PiRpc constructor receives `resolved.key === trackedModel` (wake fidelity, asserted on the CHILD's constructor args, not on engine state — spec §8).
...
**Interfaces:**
- Produces:
  - pi-lab extension: subscribes `pi.events.on("plan-mode:state", …)` and forwards each state via the extension-UI notify channel as `ctx.ui.notify("crow-state:" + JSON.stringify({kind:"plan-mode", state}))` — active ONLY when `process.env.PI_BOT_INTERACTIVE === "1"` (channel turns and personal pi are byte-identical without it). Check plan-mode's actual bus payload shape at `~/pi-lab/extensions/plan-mode/index.ts:300` and forward it verbatim. If `ctx.ui.notify` is not the real method name for the notify frame, find what produces `extension_ui_request {method:"notify"}` in pi's runner and use that.
  ...
  - Engine `control(sessionId, {planMode})`: `true` → `s.pi.promptAckOnly("/plan")`; `false` → `promptAckOnly("/plan off")` — read plan-mode's registered command surface in `~/pi-lab/extensions/plan-mode/` first and use its real on/off command strings; refused while `turn_in_progress` or hibernating (plan mode needs a live child; the drawer disables the toggle then).

- [ ] **Step 1: Failing engine tests** (fake child emits `extension_ui_request {method:"notify", message:"crow-state:{\"kind\":\"plan-mode\",\"state\":{\"active\":true}}"}`):
...
**Files:**
- Modify: `servers/gateway/perch-interactive.js`, `scripts/pi-bots/bot-world.mjs:82-86` (cwd guard)
...
- Produces:
  ...
  - **Dispatch context:** when `cardId` is set, `message()`'s FIRST turn (or spawn with an initial prompt — implement as: `spawn` stores `s.dispatchBrief`, and the first `message()` prepends it) sends `projectHeaderless` brief: `cardBriefBlock(...) + "\n\nDeliverables you produce as files go in: " + outputsDir + "\n"` — the project header already rides the bot's world/system prompt for interactive sessions? It does NOT (channel prompts carry it per-turn). So: prepend `projectContextBlock`-equivalent the same way bridge does — import `planForCard`, `cardStatus`, `boardVocab`, `projectContextBlock` from bridge.mjs via the seams (add them to `loadSeams()` :195-230 and to the `bridge` test-seam contract). Compose: `brief = header + "\n\n" + cardBriefBlock({...}) + "\n\nDeliverables you produce as files go in: <outputsDir>"`. First user message text is appended after the brief as `\n\nUser said: "<text>"` — mirroring the channel shape.
...
- [ ] **Step 1: Failing tests:**
  ...
  - dispatch first message text starts with the header+brief and ends with `User said: "hi"` (fixture the composed string; golden vs `cardBriefBlock`).
...
- Produces: notification type `"attention"`. Engine calls (via a lazy import of `servers/shared/notifications.js` + `createDbClient`) at three points:
  - turn end (non-aborted) when the turn ran ≥ `PERCH_NOTIFY_MIN_RUN_S` (env, default 30): `{type:"attention", priority:"normal", title: "<bot> replied", body: first 140 chars of reply, action_url: "/dashboard/bot-board#bird=" + sessionId}`;
...
**Files:**
- Modify: `servers/gateway/routes/perch-interactive-api.js` (ERROR_MAP :50-66 + new routes), `servers/gateway/routes/perch.js` (narrow route comment only — it already accepts perch-live; sessions route exposes `control` + `card_id` already at :521)
...
- Produces (all under `P = "/dashboard/perch-api"`, dashboardAuth-gated; fake-engine seam per the file's existing pattern):
  - `POST /bots/:id/dispatch` `{card_id}` — validates: engine ready + perchAttached (mirror :119-142); card exists in the instance-global tasks store and is not archived (open it the way `bot-board-api.js` opens tasks.db — find `resolveTasksDb` there); `await eng.checkCardFree(cardId)`; then `eng.spawn({botId, cardId})`; then write `assigned_bot` via the card service with a provenance mutation actor human (import `updateCard`/`recordMutation` from `servers/gateway/board/card-service.js` — read its exports first and call the same path `board_update_item` uses). 201 `{sessionId, threadId, state}`.
  ...
  - `POST /interactive/:sid/control` — body `{model?, thinking?, permission_mode?, plan_mode?}` → `eng.control`; `POST /interactive/:sid/cycle` → `eng.cycle`; `GET /interactive/:sid/options` → `eng.options`.
...
- Modify: `client.js` (mount + open/close + hash `#bird=`), `html.js` (drawer shell container), `css.js` (import drawer styles)
...
- Produces: right slide-over (`.bb-drawer`, `aria-modal`, ESC/backdrop close). Open paths: bird click, card bird click, `#bird=<sid>` on load (hash parsing beside the existing hash state at client.js:755-782 — read that block and extend, do not replace). Contents:
  ...
  - composer: textarea + send → `POST message` (202); while a turn runs the button relabels to the steer affordance — pi steer is NOT wired for interactive sessions (promptTurn owns the child's turn); DECISION per spec §4: composer during a turn sends **steer** — implement as `POST message` refusal today would 409 `turn_in_progress`, so the drawer greys send and shows Abort; true steer needs an engine method — add `eng.steer(sessionId, text)` → `s.pi.send({type:"steer", message:text})` fire-and-forget (no waiter; pi queues it) + a system note. Add the engine method in this task (3 lines + one test in the engine test file).

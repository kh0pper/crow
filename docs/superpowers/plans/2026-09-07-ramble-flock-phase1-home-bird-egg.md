...
test("only one incubating egg can exist (partial unique index)", async () => {
  await db.execute({ sql: "INSERT INTO ramble_eggs (egg_id, status, created_at) VALUES ('i1','incubating',1)", args: [] });
  await assert.rejects(db.execute({ sql: "INSERT INTO ramble_eggs (egg_id, status, created_at) VALUES ('i2','incubating',1)", args: [] }));
  await db.execute({ sql: "INSERT INTO ramble_eggs (egg_id, status, created_at) VALUES ('s1','shelf',1)", args: [] }); // shelf rows unaffected
});
...
- Produces (all async, `db` first; every write that changes `ramble_eggs`/`ramble_pet` calls the optional `emit(table, op, row)`):
  ...
  - `ensureIncubatingEgg(db, { now, emit }) -> egg` — returns the `status='incubating'` egg, creating one **with `status='incubating'` set explicitly** (`egg_id = randomUUID()`, warmth 0, `created_at = now`) if none exists; the partial unique index (Task 1) makes a concurrent double-create fail, so catch the constraint error and re-select.

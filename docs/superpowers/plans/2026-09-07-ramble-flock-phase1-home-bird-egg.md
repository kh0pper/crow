...
- Produces (all async, `db` first; every write that changes `ramble_eggs`/`ramble_pet` calls the optional `emit(table, op, row)`):
  ...
  - `ensureIncubatingEgg(db, { now, emit }) -> egg` — returns the `status='incubating'` egg, creating one (`egg_id = randomUUID()`, warmth 0) if none exists.
  - `creditKey(event, { now }) -> { kind, key } | null` — `visit_place` → `{ "visit_place", cell7 + ":" + isoWeek(now) }` (requires `event.cell`); `checkin` → `{ "checkin", localDay(now) }`; `meet_crow` → `{ "meet_crow", event.persona + ":" + isoWeek(now) }` (requires `event.persona`); `mark_left`/`unlock_mark` → `null` (always credited). `isoWeek(ms) -> "2026-W37"`, `localDay(ms) -> "2026-09-07"` exported (local time of the process).
  - `creditWarmth(db, event, { now, emit }) -> { credited: boolean, warmth, hatched: egg|null }` — inserts the credit row first (`INSERT OR IGNORE`; `rowsAffected === 0` → `{ credited:false }`), adds the weight to the incubating egg (clamped at `hatch_at`), then `hatchIfReady`.
  - `hatchIfReady(db, { now, emit }) -> egg|null` — when the incubating egg's warmth ≥ `hatch_at`: `species = ROSTER[crypto.randomInt(ROSTER.length)]`, `seed = crypto.randomInt(0, 2**32)`, `status='hatched'`, `hatched_at=now`; if `ramble_pet.active_egg_id` is null set it to this egg; create the NEXT incubating egg (so the loop continues); return the hatched egg.
...
**Interfaces:**
- Produces: `FEED_DELTAS` gains `checkin: +5`, `chore: +8`, `mark_left: 0` (existing `visit_place +15`, `unlock_mark +10`, `meet_crow +20`, `quiet_tick −10` unchanged); `CHORES = ["feed","preen","play"]`; `doChore(db, kind, { now, emit }) -> { done: boolean, chores: { day, feed, preen, play }, pet }` — first completion per local day writes `ramble_pet.chores_json` and feeds `{ type: "chore" }`; a repeat returns `done:false` with no change; a new day resets the three flags; `petState` gains `chores` (today's flags, day-rolled) and `active_egg_id`. Every `ramble_pet` write calls `emit("ramble_pet", "update", row)` when an `emit` is passed (`feed` and `petState`'s decay write included — add the `{ emit }` option to both).

- [ ] **Step 1: Add tests** to `tests/ramble-pet.test.js`:

```js
test("chores: once per day each, +8 energy, day rollover resets", async () => {
  const db = await freshDb(); // helper already in the file (or create one like the others)
...
test("pet writes emit when a hook is given", async () => {
  const db = await freshDb(); const seen = [];
...
- `applyRambleEgg(db, op, row, lamportTs)`: key `egg_id`; LWW skip when `lamportTs < localTs`; delete by key; upsert `INSERT … ON CONFLICT(egg_id) DO UPDATE SET status, warmth, species, seed, found_cell, found_week, from_crow_id, hatched_at, lamport_ts` (columns from the wire row ∩ schema, never binding undefined). **Invariant:** never overwrite a local non-null `species`/`seed` with null (a hatched bird can't be un-hatched by a stale peer update).
- `applyRamblePet(db, op, row, lamportTs)`: key `owner` (always `'self'`); LWW; upsert the columns `mood, energy, last_fed_at, places_week, unlocks_week, crows_week, week_start, active_egg_id, chores_json, lamport_ts`; deletes are ignored (the pet row is never deleted).
...
test("outbox door: an egg write with no manager queues and is stamped", async () => {
  ...
  const res2 = await emitOrQueue(null, a, "ramble_pet", "update", { ...(pet.rows[0] || { owner: "self", mood: "happy", energy: 60 }) });
...
test("apply door: egg insert, LWW, hatch survives a stale update, delete", async () => {
  ...
  got = await b.execute({ sql: "SELECT status, species, seed FROM ramble_eggs WHERE egg_id='e1'", args: [] });
  assert.equal(got.rows[0].species, "crow"); assert.equal(got.rows[0].seed, 77);
...
(add `ensureIncubatingEgg` to the imports.)
...
- Transport `drainMarks`: resolve `activeBird(db)` (from `eggs.js`, loaded with the other bundle modules) ONCE per drain and pass it as `bird` to every `markToEvent`. On `onEvent` insert success, additionally `feedAll(db, { type: "meet_crow", persona: event.pubkey }, { emit, onHatch: (egg) => bus.emit("ramble:hatched", { egg_id: egg.egg_id, species: egg.species, seed: egg.seed }) })` (best-effort, try/catch).
...
`tests/ramble-transport.test.js`: seed a hatched active bird (insert into `ramble_eggs` + set `ramble_pet.active_egg_id`), drain, assert the published event's content has `bird`; feed a remote event and assert `ramble_credits` gained a `meet_crow` row and a second identical event does not add another.
...
```bash
git commit bundles/ramble/server/nostr-map.js bundles/ramble/server/marks.js servers/gateway/boot/ramble-transport.js tests/ramble-nostr-map.test.js tests/ramble-marks.test.js tests/ramble-transport.test.js -m "feat(ramble): active bird rides on public caws/marks; meet_crow credits on receipt"
git show --stat HEAD
```
...
**Interfaces:**
- Routes (all under the existing `/api/ramble` auth scope): `GET /api/ramble/egg` → `eggState`; `POST /api/ramble/egg/checkin` → `feedAll({type:"checkin"})` → `{ credited, warmth, hatched }`; `POST /api/ramble/pet/chore { kind }` → `doChore` (400 on bad kind); `GET /api/ramble/pet` → `petState` + `bird` + `egg: { percent }`; `GET /api/ramble/bird/:species/:seed.svg` (auth-scoped under `/api/ramble`) → `image/svg+xml` of `<svg viewBox="0 0 200 200">` + `drawBird(rollGenome(seed, species), req.query.mood)` with `Cache-Control: public, max-age=86400` and 400 for an invalid pair; `GET /ramble/static/bird-svg.js` → serves `BUNDLE_DIR/server/bird-svg.js` (path-guarded, `text/javascript`).
- Existing routes: `POST /api/ramble/marks` passes `bird: activeBird` into `createMark` and feeds `mark_left`; `POST /api/ramble/unlock` and `POST /api/ramble/area` switch to `feedAll`; every `feedAll` in routes gets `onHatch: (egg) => bus.emit("ramble:hatched", { egg_id, species, seed })`.
...
- [ ] **Step 1: Add tests** — panel: egg route shape; check-in twice (second `credited:false`); chore twice + bad kind 400; pet route carries `chores`; `bird.svg` 200 `image/svg+xml` containing `<g`, and 400 for `dodo`; `bird-svg.js` served 200 with `text/javascript`; posting a mark increases warmth (compare `GET /api/ramble/egg` before/after). stream: emitting `ramble:hatched` writes one `event: ramble-hatched` frame with exactly `egg_id, species, seed`; `fireClose()` removes both listeners.
...
- Client rules: plain script, zero backticks, all dynamic text via `textContent`, `fetch` with JSON (the layout attaches CSRF), `EventSource` with named listeners `ramble-nearby` and `ramble-hatched`, map `touchstart/touchend` → `Crow.setPullToRefresh(false/true)` (keep), `moveend` debounced → `POST /api/ramble/area` + marks refresh; pins: open marks = `L.marker` with bubble popup; locked = dashed `L.circleMarker` at `approx_*` with "walk N m to read" + Unlock button; caws = bubble + a 40 px bird drawn from `bird_species/bird_seed` via `RambleBird.mountBird`; nearby list mirrors the pins. Perch bird is `mountBird(#rb-perch-bird, rollGenome(seed, species), mood)`. Reduced-motion respected.
...
**Interfaces:**
- In `tamagotchiJs`: after the existing boot, `fetch('/api/ramble/pet', { credentials: 'same-origin' })` (best-effort: any non-200 or network error → do nothing); if the JSON has `bird` (`{ species, seed }`), load `/ramble/static/bird-svg.js` by injecting a `<script>` once (guard on `window.RambleBird`), then `RambleBird.mountBird(document.getElementById('crow-tama'), RambleBird.rollGenome(seed, species), currentMood)` and set `viewBox="0 0 200 200"`; `updateCrowMood(health)` re-mounts with the mapped mood (`health ≥ 60 → happy`, `30–59 → tired`, else `alarmed`) when a bird is mounted, else keeps the existing class logic. The dropdown `onclick` and the bubble/exclaim elements must keep working (re-append them after `innerHTML` replacement, or draw into a child `<g id="crow-tama-bird">` and hide the original `.crow-body-group` instead of replacing the whole SVG — prefer the latter).

- [ ] **Step 1: Write the failing test** — `tests/ramble-header-bird.test.js`: import `tamagotchiJs` from the notifications module, get the script string for `"en"`, `new Function(src)` must NOT throw at parse time (wrap: `assert.doesNotThrow(() => new Function(src))`), and the source contains `/api/ramble/pet`, `/ramble/static/bird-svg.js`, `RambleBird.mountBird`, and `crow-tama-bird`. Also assert the existing `tests/nest-xss.test.js` and any notifications tests still pass (run them).
...
_(plan-review rounds are appended here)_

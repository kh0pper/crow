...
Executed subagent-driven (fresh implementer + spec+quality review per task; opus final whole-branch review). Tasks 1–4 + the Task-5 manifest-drift fix all landed with clean per-task reviews. Commits: `ce2a6b73` (cursor), `36b5e602` (upsertFullContact + tool), `5ba2afef` (invite_accepted promote), `57eb19fa` (repair form + i18n), `002c5ea7` (tool-manifest registration), `ca54ffeb` (security fix, below). Full suite **939/939**; gateway boots clean (`[sharing] Subscribed to incoming Nostr messages`, 3 relays).

**Final whole-branch review (opus): NOT READY → 1 CRITICAL, now FIXED + re-verified.**
- **CRITICAL (fixed `ca54ffeb`):** `handleInviteAccepted` promoted on the payload's *claimed* secp/ed/crowId without checking they came from the authenticated signer. Because a Nostr pubkey is public, a stranger could send a forged `invite_accepted` claiming a victim's secp key and promote/hijack a gated message-request row (forging a "trusted victim" contact whose peer-sync identity is the attacker's). The plan's security note asserted the `event.pubkey` binding but the code never enforced it. Fix: thread the authenticated `senderPubkey` (`event.pubkey`) through `onInviteAccepted`→`handleInviteAccepted` and gate `normalizePubkey(payload.secp256k1Pub) === normalizePubkey(senderPubkey)` before promoting. Verified honest-flow-transparent (`crow_accept_invite` signs with the same secp key it sends) + a forged-key negative test (fails pre-fix, passes post-fix). Re-reviewed FIX COMPLETE.
- **MINOR (ledger follow-ups, non-blocking):** (1) the merge branch reassigns only `messages`, not other contact-scoped children (`shared_items`/`contact_group_members`/`project_members`) — empty in practice since the losing row is always a `req:` message-request that never accrues those; degrades safely. (2) `isPlaceholderName` treats a deliberately-chosen `crow:`-prefixed display name as a placeholder (vanishingly unlikely). (3) `persistIncomingCursor` writes one upsert per received DM (extra write on the `:3001` DB — acceptable, watch under load). (4) a sender-controlled future `created_at` can push the cursor forward, but `readIncomingSince` clamps reads to `now-24h` so it only degrades to the fixed 24h backfill.

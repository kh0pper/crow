...
| D4 | Avatar transport | The avatar travels as a small inline image (`data:` URI), never a URL: the browser shrinks an upload to a 128 px square; it rides the pairing handshake both ways, and a `crow_profile` message goes to every full contact when the profile changes. |

...
| `unified` | `true` when Vulkan `deviceType` is `INTEGRATED_GPU`, **or** amdgpu sysfs shows `mem_info_vram_total ≤ 2048 MiB` alongside a `mem_info_gtt_total`. `false` for a discrete GPU. | no GPU |
| `gttTotalMb`, `gttUsedMb` | the first `/sys/class/drm/card*/device/mem_info_gtt_{total,used}` | not amdgpu, or not linux |
...
The discrete path is unchanged. When `probe.unified === true`:

- **The ceiling** is `gttTotalMb`, or `ramTotalMb` when GTT is unknown. A quant
  whose `min_ram_mb` exceeds the ceiling is `wont_fit`, because it can never
  run on this box.
- `min_ram_mb <= ramAvailableMb` → `fits`.
- Otherwise → `tight`: it fits the box, but only after other resident models
  are stopped.
...
- **Operator surface (D5):** a CLI, `scripts/models-runtime-override.mjs`,
  ...
  - It resolves the data dir the same way the gateway does (`CROW_HOME` /
    `CROW_DATA_DIR`).
...
- **`fitBadge` unified matrix:**
  ...
  - between available and GTT → `tight`;
  - above GTT → `wont_fit`;
  - GTT unknown → the ceiling is `ramTotalMb`;
...
- **CLI:** `list`, `set` and `clear` against a temp `CROW_HOME`.
- **Launch:**

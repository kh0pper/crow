...
**Implementation review (post-build): opus holistic data-safety review — FIX FIRST → resolved.** Per-task gates (spec-compliance + code-quality, both tasks) passed with minor fixes. The opus cross-cutting pass found one HIGH interaction no per-slice reviewer owned:
- **F1 (deploy-sequencing silent overwrite):** the conflict INSERT names the new `op` column; on a host that pulls code and restarts WITHOUT running init-db, the INSERT throws and `_checkConflict`'s catch fell through to `"apply"` — a stale remote update would silently overwrite newer local data (worse than main). Fixed three ways: (a) gateway-boot migration `ensureSyncConflictsOpColumn` in `servers/gateway/migrations.js` (PRAGMA-checked, idempotent); (b) `_insertConflictRow` helper degrades to the legacy column set when `op` is missing (both call sites); (c) once the conflict verdict is determined, logging failure can no longer flip the verdict — the log+notify has its own guard and `_checkConflict` returns `"skip"` regardless (the outer error path, where the verdict is undetermined, still defaults to `"apply"` as settled). Pinned by test 14 (legacy DB without `op`: stale update skipped AND conflict logged).
- **F2 (accepted risk, documented):** an operator-initiated restore can interleave with a concurrently-arriving inbound edit to the same row between the stale-snapshot guard read and the restore UPDATE (different async flow than the same-request TOCTOU waived in §6 step 1). Millisecond window, operator-timed, recoverable (the next divergent edit re-logs a conflict). Accepted.
- F3 (transient-apply skip-and-advance) verified pre-existing on main, not introduced; per-entry checkpointing is strictly safer. Mixed-version fleet, crash windows, and the restore guard ordering all attested SAFE.

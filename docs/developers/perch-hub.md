...
A perch turn is one `handleInbound()` call, like a Gmail turn, and it runs **in-process** in the gateway because only that gives a streaming `sendReply` to push down SSE. Board dispatch also calls `handleInbound()` with the same contract, but out-of-process: the board enqueues a `bot_jobs` row instead of spawning it directly, and the pi-bots worker (not the gateway) is what runs it. The bridge is imported lazily so gateway boot stays light.

...
def insert_rows(cursor, table_name, rows):
    ...
    # Convert dict rows to list of tuples in the same column order
    col_order = list(rows[0].keys())
    tuple_rows = [tuple(row[c] for c in col_order) for row in rows]
    cursor.executemany(f"INSERT INTO {table_name} ({cols}) VALUES ({placeholders})", tuple_rows)
    return len(rows)

...
// Warm the resolved model bundle before spawning pi. The pi-bots host is a
// SEPARATE process from the gateway (which owns the gpu-orchestrator), so it warms
// over HTTP via the gateway's POST /llm/acquire instead of running a second
// orchestrator. Target = the GPU-owning gateway on loopback (PIBOT_WARM_GATEWAY_URL,
// else CROW_GATEWAY_PORT, else :3001). Without this an on-demand model (:8003) is
// cold and pi returns "Connection error" → "(no reply)". See memory
// pibot-bridge-no-model-warming.
...
async function warmModel(provider, log = () => {}) {
  if (!provider) return;
  try {
    ...
    // Best-effort: a failed warm is not fatal — pi will surface the real
    // connection error if the model is genuinely unavailable.

...
/**
 * Crow Research Pipeline MCP Server
 *
 * Manages research projects, sources, citations, and notes.
 * All sources are tracked with APA citations, retrieval metadata,
 * and verification status. Full-text searchable via SQLite FTS5.
 */
...
if (!existsSync(DB_PATH)) {
  console.error(`Database not found at ${DB_PATH}. Run 'npm run init-db' first.`);
  process.exit(1);
}
...
// --- Helper: Generate APA Citation ---

function generateAPA({ authors, title, publication_date, publisher, url, source_type, doi }) {
  // If a manual citation is provided, use it. Otherwise generate best-effort APA.
...
server.tool(
  "add_source",
  "Add a research source with full metadata and automatic APA citation generation. Use this whenever you encounter information worth documenting - web articles, papers, interviews, scraped data, etc.",
  {
    ...
    retrieval_method: z.string().optional().describe("How the source was obtained (web search, API, manual, etc.)"),
    tags: z.string().optional().describe("Comma-separated tags"),
...
server.tool(
  "search_sources",
  "Full-text search across all research sources - titles, authors, abstracts, summaries, and content.",
  {
...
server.tool(
  "get_source",
  "Get full details of a specific source by ID, including all metadata and citation.",
...
server.tool(
  "verify_source",
  "Mark a source as verified or unverified, with optional notes about the verification.",
  {
    id: z.number().describe("Source ID"),
    verified: z.boolean().describe("Whether the source is verified"),
    notes: z.string().optional().describe("Verification notes (e.g., 'Confirmed via Wayback Machine')"),
  },
...
server.tool(
  "list_sources",
  "List sources with filtering options. Good for browsing the research database.",
  {
...
server.tool(
  "add_note",
  "Add a research note - can be attached to a project, a source, or both. Use for quotes, summaries, analysis, questions, and insights.",
  {
...
server.tool(
  "research_stats",
  "Get statistics about the research database - project counts, source counts by type, verification status.",
  {},
  async () => {

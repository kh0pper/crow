/**
 * Crow's Nest Panel — Podcast subscriber: subscribe to feeds, browse episodes, playlists, audio player
 */

import { escapeHtml, statCard, statGrid, section, badge, dataTable, formField } from "../shared/components.js";
...
function parseRSS(xml) {
  // Lightweight RSS parser — extracts channel info and episodes
  ...
  async handler(req, res, { db, layout }) {
    ...
    if (req.method === "POST") {
      ...
      if (action === "subscribe") {
        ...
        if (!feedUrl) return res.redirect("/dashboard/podcast?error=empty");
        ...
        try {
          ...
          if (subId && items.length > 0) {
            // Insert episodes (limit to 50 most recent)
          ...
          return res.redirect(`/dashboard/podcast?error=${encodeURIComponent(err.message)}`);
        }
        return res.redirect("/dashboard/podcast");
      }
        ...
        return `<div class="card" style="padding:0.75rem;margin-bottom:0.5rem">
          <div style="display:flex;justify-content:space-between;align-items:start;gap:0.5rem">
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:0.8rem;color:var(--crow-text-muted)">${escapeHtml(ep.podcast_title)} · ${escapeHtml(pubDate)} ${durationStr ? `· ${escapeHtml(durationStr)}` : ""}</div>
            </div>
    ...
    return layout({ title: "Podcasts", content });
  },
};

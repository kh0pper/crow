...
export const translations = {
  ...
  "extensions.dockerUnavailableDesc": { en: "Extensions that deploy containers can't be installed until Docker is installed and its daemon is running. On a Crow appliance, re-run the install script (scripts/crow-install.sh) — it installs Docker. Extensions that only connect to existing services still work.", es: "Las extensiones que despliegan contenedores no se pueden instalar hasta que Docker esté instalado y su demonio en ejecución. En un equipo Crow, vuelve a ejecutar el script de instalación (scripts/crow-install.sh), que instala Docker. Las extensiones que solo se conectan a servicios existentes siguen funcionando." },

...
const locked = { mark_id: "m1", reveal: "locked", anchor_kind: "geo", lat: 30.2672, lon: -97.7431, accuracy_m: 75, geohash: "9v6m2a", content_text: "secret spot", content_kind: "none", bird_species: "crow", bird_seed: 5 };
...
test("a locked teaser keeps bird_species/bird_seed (D5: whose bird left it is not a secret)", () => {
  const teasered = teaser(locked);
  assert.equal(teasered.bird_species, "crow");
  assert.equal(teasered.bird_seed, 5);
});

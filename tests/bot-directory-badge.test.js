...
test("getUnifiedConversationList carries isBot for bot contacts", async () => {
  ...
  // verified: P2/C4 Task 5 threads c.verified through getUnifiedConversationList's
  // peer-row SELECT — the minimal schema here must carry the column or the SELECT
  // throws (swallowed by the function's try/catch, silently dropping all peer rows).

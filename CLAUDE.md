...
bundles/motioneye/             → motionEye: lightweight web frontend for the Motion daemon. Pi-friendly, USB webcam + RTSP support, motion-triggered recording. Iframe-only (no MCP tools in v1); install alongside Frigate if you also want AI detection
...
- **research_projects** — Legacy projects table. Still real + writable (12+ legacy INSERT callsites in tree); a forward trigger mirrors every write into `project_spaces`. → **research_sources** → **research_notes** — Foreign keys with `ON DELETE SET NULL`. Both child tables gained `uuid`, `origin_instance_id`, `archived_at` columns in the M0/M1 redesign (see Project Spaces below).
- **project_spaces** — First-class shareable project (Phase 1 redesign, 2026-05-26). One row per shareable space with `slug` (unique kebab), `uuid`, `name`, `description`, `type` (free text — `general` / `research` / `data_connector` / `learner_profile` / `bot_workspace`), `status` (active/paused/completed/archived CHECK), `owner_contact_id`, `workspace_dir` (absolute fs path), `storage_prefix` (MinIO key prefix), `tasks_db_uri`, `db_path` (NULL in Phase 1; reserved for future per-project DB split), `origin_instance_id`, `archived_at`. Indexes on slug/status/type. **The legacy `research_projects` table is being phased out** — new MCP tools (`crow_workspace_dir`, `crow_project_get`, `crow_project_capabilities`, `crow_add_member`, `crow_remove_member`, `crow_list_members`, `crow_audit_log`) and the bot bridge write directly to `project_spaces`; the forward trigger keeps `project_spaces` in sync with legacy `research_projects` writes during the transition.
- **project_members** — ACL membership for project spaces. `role IN ('owner','editor','viewer','guest')` sets default capability bundles; `capabilities` JSON column overrides per-member (e.g., `{"invoke_bot":false,"query_backend:tea-data":true}`). Two partial UNIQUE indexes: one for local-owner row (`contact_id IS NULL`), one for remote members (`contact_id IS NOT NULL`) — SQLite's NULL-distinct behavior makes a plain composite unique insufficient. `mode` column = `clone` for clone-shared rows; `revoked_at` is soft-revoke. ACL helper at `servers/shared/project-acl.js` (`assertCapability`, `effectiveCapabilities`, `assertProjectExists`).
- **project_audit_log** — Timeline of mutating events on a project (`actor_type IN ('local','contact','bot','system')`, `action` is a string like `member.add`, `source.add`, `bot.invoke`, `file.upload`, `share.send`). Best-effort append (failures never break the primary action).
...
- **storage_files** — S3 object metadata (key, name, MIME, size, bucket, reference to other items). `project_id` column (added 2026-05-26) is the authoritative ACL boundary for new uploads; legacy rows (`project_id IS NULL`) keep pre-redesign behavior. Reference-target consistency is enforced at upload: if `reference_id` points at a `research_sources`/`research_notes` row pinned to a project, the upload's `project_id` must match.
...
- `motioneye.md` — motionEye: lightweight Motion daemon frontend. No MCP tools — routes user to the iframe Web UI at /dashboard/motioneye, and redirects to frigate.md if the user needs AI detection or MCP-callable camera ops.
